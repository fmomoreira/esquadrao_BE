  ...
  return new Promise(async (resolve, reject) => {
    try {
      (async () => {
        ...
        // Mock para substituir makeInMemoryStore que não existe mais na versão atual do Baileys
        const store = {
          bind: (_conn: any) => {},
          loadMessages: async (_jid: string, _count: number) => ({ messages: [], cursor: null }),
          loadMessage: async (_jid: string, _id: string) => null,
          search: async (_jid: string, _query: string, _count: number) => [],
          chats: {
            all: () => []
          },
          writeToFile: () => {},
