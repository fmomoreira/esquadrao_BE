...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  let ticket_id_number = ~~Number(ticketId);
  let tag_id_number = ~~Number(tagId);

  try {
    const ticketTag = await TicketTag.create({ ticketId: ticket_id_number, tagId: tag_id_number });
